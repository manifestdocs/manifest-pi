...
import { createPlanModeController } from '../dist/hooks/plan-mode.js';
import {
  isSafeCommand,
  extractTodoItems,
  markCompletedSteps,
} from '../dist/hooks/plan-utils.js';
...
3. **PLAN** -- You are now in read-only plan mode. Use read, bash, grep, find, ls + manifest read tools to understand the codebase. Produce a numbered plan under a "Plan:" header. Do NOT attempt to write or edit files.
...
export default async function featureWorkflow(pi: ExtensionAPI): Promise<void> {
  ...
  // ── Plan Mode: Filter stale context messages ──

  pi.on(
    'context',
    async (event: ContextEvent): Promise<ContextEventResult | void> => {
  ...
  // ── System Prompt + Plan Mode Context ──────────
  ...
  pi.on(
    ...
    async (
      ...
      const systemPrompt = event.systemPrompt + MANIFEST_CONTEXT;
...
After completing each step, include a [DONE:n] tag (e.g., [DONE:1]) so progress is tracked.`,
              display: false,
            },
          };
        }
  ...
  // ── Plan Mode: Track [DONE:n] markers ──────────
  ...
  function startExecution(todos: ReturnType<typeof planController.getTodoItems>, ctx: ExtensionContext): void {
  ...
  // ── Plan Mode: Extract plan + show menu ────────
  ...
  pi.on('agent_end', async (event: AgentEndEvent, ctx: ExtensionContext) => {
    ...
    if (todos.length > 0) {
      const todoListText = todos
        .map((t, i) => `${i + 1}. ${t.text}`)
        .join('\n');
      ctx.ui.notify(`Plan (${todos.length} steps):\n${todoListText}`, 'info');
    }

    // In yolo mode, skip approval and go straight to execution
    if (yoloMode) {
  ...
  pi.on('session_start', async (_event: any, ctx: ExtensionContext) => {
    ...
    // Restore from --plan flag
    if (pi.getFlag('plan') === true) {
      planController.enter(pi, ctx);
    }

    // Restore persisted plan state on session resume
    ...
    const planModeEntry = entries
      ...
      .pop() as { data?: { state: string; todos?: any[] } } | undefined;
    ...
    if (planModeEntry?.data) {
      ...
      if (savedState === 'plan') {
        ...
        // Re-scan messages after last plan-mode-execute to rebuild completion state
        let executeIndex = -1;
        ...
        for (let i = executeIndex + 1; i < entries.length; i++) {
          ...
          if (
            entry.type === 'message' &&
            'message' in entry &&
            isAssistantMessage((entry as any).message)
          ) {
